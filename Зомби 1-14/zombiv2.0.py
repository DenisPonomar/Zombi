...
def Snaryad():
    ...
    if vystrel == False:
        ...
        global x_cos
        global y_sin
        pygame.draw.circle(screen, [255, 255, 255], [int(50+math.cos(angle*math.pi/180)*125+x_snaryad), int(600-math.sin(angle*math.pi/180)*125-y_snaryad)], 10, 0)
        x_snaryad = 0
        y_snaryad = 0
        x_speed = 10
        y_speed = 10
    ...
    if keystate[pygame.K_UP]:
        x_cos = math.cos(angle*math.pi/180)
        y_sin = math.sin(angle*math.pi/180)

    if vystrel == True:
        x_snaryad = x_snaryad + x_speed * x_cos
        y_snaryad = y_snaryad + y_speed * y_sin - gravity
        gravity = gravity + 0.1
        pygame.draw.circle(screen, [255, 255, 255], [int(50+x_cos*125+x_snaryad), int(600-y_sin*125-y_snaryad)], 10, 0)
    if int(600-y_sin*125-y_snaryad) > 700:
        vystrel = False
...
while running:
    ...
    keystate = pygame.key.get_pressed()
    if keystate[pygame.K_UP]:
         pygame.draw.circle(screen, [255, 255, 255], [int(50+math.cos(angle*math.pi/180)*125), int(600-math.sin(angle*math.pi/180)*125)], 50, 0)
         vystrel = True
    pygame.display.flip()
    clock.tick(50)
pygame.quit()
