...
class MyWindowClass(QtGui.QMainWindow, QtGui.QWidget, form_class):
    def __init__(self, parent=None):
        ...
        for i in range(chislo_Vse_Game_Name):
            ...
            button_score.setGeometry(220, (270+i*50), 150, 35)
            button_score.setStyleSheet("background: #111")
...
screen = pygame.display.set_mode((1366, 700),pygame.FULLSCREEN)
sol_angle = 45
stars_radius = []
cos_stars_angle = []
sin_stars_angle = []
...
def Zemlya():
    ...
    if col != col_temp:
        ct = copy.copy(lug)
        ct.fill((255-col, 255-col, 255-col, 0), special_flags=pygame.BLEND_RGBA_SUB)
        col_temp = col
        
    screen.blit(ct, (0,500))
    #screen.blit(lug, (0, 550))
    #pygame.draw.rect(screen, [0, col, 0], [0, 650, 1366, 50], 0)
kol_snar = 20000

def Pushka():
    pygame.draw.rect(screen, [col, col, col], [50, 550, 100, 125], 0)
    pygame.draw.circle(screen, [col, col, col], [100, 550], 50, 0)
    pygame.draw.rect(screen, [0, 0, 0], [50, 550, 100, 2], 0)
    pygame.draw.ellipse(screen,[col, col, col], ((50,660),(100,20)), 0)
